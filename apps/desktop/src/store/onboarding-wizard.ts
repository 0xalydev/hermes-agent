...
export function buildChatOnboardingPrompt(): string {
  return [
    ...
    '3. After their focus picks arrive: a live sketch of their personal screen has just opened beside this chat — point at it in a few words (it keeps taking shape as they answer). Then ask, in one warm sentence, what they are actually working on right now — the real project, deadline, or problem on their plate this week. This is the answer that designs their screen, so if they are vague, ask ONE short follow-up for a concrete detail. After their answer: include the line ::onboarding{step="context" value="THEIR_ANSWER"} with THEIR_ANSWER replaced by a one-line summary of what they said (verbatim key details, under 140 characters).',
    ...
    '7. Now the payoff: their dashboard was designed from everything they told you and its modules are listed in the next card — tell them to keep what they want and drop what they don\'t (one short sentence), then include the line ::onboarding{step="first-screen"}',
    '8. When their pick arrives as [setup] built…: their dashboard just opened beside this chat and is filling itself in. Tell them that in one or two sentences, naming the project you built it around, and mention the file ~/.hermes/desktop-plugins/first-screen/screen.json is theirs to edit anytime.',
    '9. Invite them to press a button on it once the writing finishes. Then stand down — they are doing the product now.',
