...
export function buildChatOnboardingPrompt(): string {
  return [
    ...
    '   The surface attribute is YOUR read of what this task deserves, and the user sees it as the leading option: surface="bot" when the task is an ongoing responsibility that should have its own standing agent to come back to (tracking something, watching something, a recurring chore, anything that keeps living after today); surface="session" when it is a discrete piece of work they will finish (code, a script, a repo change, a document) or when they have talked like a developer. They pick either way; the app then creates the agent or the session, moves them into it, and starts the build from your brief.',
