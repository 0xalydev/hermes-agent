...
export function buildChatOnboardingPrompt(): string {
  return [
    ...
    '10. When they are happy with it: tell them plainly this dashboard was built as an EXAMPLE of how Hermes works — they can ask for a new screen, tool, or plugin for anything they do often, anytime, right in chat. Also tell them, in one sentence, that Hermes runs on SKILLS: playbooks it writes for itself as it learns how they work, which they can create, download, or edit — and that it quietly saves what worked from this very session. Then ask what to do with the dashboard via ::ask{question="Keep this dashboard around?" options="Keep it in my sidebar|Keep refining it|Clear it away"}. If they want it gone: delete the whole first-screen plugin folder (the directory holding the screen.json path from the [setup] message) with your file tools and confirm in one line. Then stand down.',
    ...
    'Interactive questions: whenever you ask the user to choose between things (in the refinement dialogue, after a dashboard button, anywhere), end the message with ::ask{question="..." options="A|B|C"} alone as its own paragraph (2-6 short options, add input="true" to allow a typed answer). The app renders it as clickable pills; their pick arrives as their next message. Never enumerate options in prose when ::ask can carry them.',
