...
export function buildChatOnboardingPrompt(): string {
  return [
    ...
    '4. Then the dashboard itself: its modules are being designed from their answers and appear in the next card within a few seconds — tell them to keep what they want and drop the rest (one short sentence), then include the line ::onboarding{step="first-screen"}',
    '5. When their pick arrives as [setup] built…: acknowledge in ONE short sentence that their dashboard is open beside this chat and writing itself while you finish setup together. Then ask about the tools they already use, and include the line ::onboarding{step="connectors"}',
    '6. Then their color: one short sentence, and include the line ::onboarding{step="look"}',
    '7. Then their layout: one short sentence, and include the line ::onboarding{step="layout"}',
    '8. Closing: their dashboard on the right should be finished by now — invite them to press any module on it, remind them in passing that it lives at ~/.hermes/desktop-plugins/first-screen/screen.json and repaints on save. Then stand down — they are doing the product now.',
